...
Camera::Camera(std::shared_ptr<rclcpp::Node> n, std::string cam_info_topic):n_(n) 
{
	calib_K = cvMat(3, 3, CV_64F, calib_K_data);
	calib_D = cvMat(4, 1, CV_64F, calib_D_data);
	memset(calib_K_data,0,sizeof(double)*3*3);
	...
	sub_ = n_->create_subscription<sensor_msgs::msg::CameraInfo>(cameraInfoTopic_, 1, &Camera::camInfoCallback);
	getCamInfo_ = false;
}
...
void Camera::SetCameraInfo(const sensor_msgs::msg::CameraInfo &camInfo)
{
    cam_info_ = camInfo;
...
void Camera::camInfoCallback(const sensor_msgs::msg::CameraInfo::SharedPtr msg)
  {
    if (!getCamInfo_)
    {
        SetCameraInfo(*msg);
