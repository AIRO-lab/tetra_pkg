...
void VirtualLayer2::onInitialize()
{
  ...
  zone_sub_ = node_->create_subscription<tetra_msgs::msg::Zone2>(
    ...
    std::bind(&VirtualLayer2::zoneCallback, this, std::placeholders::_1));
  obstacles_sub_ = node_->create_subscription<tetra_msgs::msg::Obstacles2>(
    obstacle_topics_name, rclcpp::SystemDefaultsQoS(),
    std::bind(&VirtualLayer2::obstaclesCallback, this, std::placeholders::_1));
...
bool VirtualLayer2::parseFormListFromString(const std::string & form_string){
  std::string error;
...
rcl_interfaces::msg::SetParametersResult VirtualLayer2::dynamicParametersCallback(std::vector<rclcpp::Parameter> parameters)
{
...
void VirtualLayer2::updateCosts(nav2_costmap_2d::Costmap2D &master_grid, int min_i, int min_j, int max_i, int max_j)
{
  std::lock_guard<nav2_costmap_2d::Costmap2D::mutex_t> guard(*getMutex());
  if (!enabled_) {
    return;
...
void VirtualLayer2::setPolygonCost(nav2_costmap_2d::Costmap2D &master_grid, const Polygon &polygon, unsigned char cost,
...
void VirtualLayer2::zoneCallback(const tetra_msgs::msg::Zone2::SharedPtr zone_msg)
{
...
void VirtualLayer2::obstaclesCallback(const tetra_msgs::msg::Obstacles2::SharedPtr obstacles_msg)
{
  if (_clear_obstacles) {
...
geometry_msgs::msg::Point VirtualLayer2::getRobotPoint()
{
